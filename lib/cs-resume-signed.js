(function( root, factory ) {
...
})(window || this, function(XMLHttpRequest, SparkMD5, Q, FileReader, Blob, DOMParser) {
  ...
  const uploadUriCache = {} // fileHash -> uploadURI

  return {
    MD5Hash: md5Promise,
    Uploader: function ({signedURI, uploadURI, file}) {
      
      if (!signedURI && !uploadURI) {
        throw "EMPTY_URI"
      }

      const urlCheckExpirted = new URL(signedURI || uploadURI)
      const expiration = parseInt(urlCheckExpirted.searchParams.get('Expires')) * 1000
      const compareDate = new Date().getTime() + new Date().getTimezoneOffset() * 60000
      if (compareDate >= expiration) {
        throw "URI_EXPIRED"
      }
